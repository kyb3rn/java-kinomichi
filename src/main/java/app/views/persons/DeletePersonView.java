...
public class DeletePersonView extends View {
    ...
    public DeletePersonView(List<Person> sortedPersons, PersonDataManager personDataManager) {
    ...
    public Event render() {
        ...
        System.out.println(TextFormatter.italic("Entrez l'identifiant (#) d'une personne pour la supprimer."));
        ...
        try {
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                ...
                confirmationSimpleBox.addLine("Êtes-vous sûr de vouloir supprimer la personne " + TextFormatter.bold(selectedPerson.toString()) + " ?");
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                ...
                if (normalizedInput.equals("N")) {
                    selectedPersonId.set(-1);
                }
            });

            if (selectedPersonId.get() == -1) {
            ...
            if (response instanceof SortColumnCommand sortColumnCommand) {
                return new CallUrlEvent("/persons/delete/select/sort/" + this.buildSortPathSegment(sortColumnCommand));
