...
public class DeleteLodgingReservationView extends View {

    // ─── Properties ─── //

    private final int campId;
    private final int lodgingId;
    private final List<LodgingReservation> sortedLodgingReservations;
    private final LodgingReservationDataManager lodgingReservationDataManager;

    // ─── Constructors ─── //

    public DeleteLodgingReservationView(int campId, int lodgingId, List<LodgingReservation> sortedLodgingReservations, LodgingReservationDataManager lodgingReservationDataManager) {
        this.campId = campId;
        this.lodgingId = lodgingId;
        this.sortedLodgingReservations = sortedLodgingReservations;
        this.lodgingReservationDataManager = lodgingReservationDataManager;
    }
    ...
    public Event render() {
        ...
        try {
            Table table = ModelTableFormatter.forList(this.sortedLodgingReservations);
            System.out.println();
            table.display();
        ...
        System.out.println(TextFormatter.italic("Entrez l'identifiant (#) d'une réservation d'hébergement pour la supprimer."));

        try {
            AtomicInteger selectedLodgingReservationId = new AtomicInteger();
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                switch (command) {
                    ...
                    case SortColumnCommand sortColumnCommand -> {
                        int columnCount;
                        try {
                            columnCount = ModelTableFormatter.getColumnCount(LodgingReservation.class);
                        } catch (UnimplementedModelTableException e) {
                ...
                int lodgingReservationId;
                try {
                    lodgingReservationId = Integer.parseInt(input);
                ...
                LodgingReservation selectedLodgingReservation = this.lodgingReservationDataManager.getLodgingReservationWithExceptions(lodgingReservationId);

                if (selectedLodgingReservation == null) {
                    throw new InvalidMenuInputException("Aucune réservation d'hébergement ne porte l'identifiant '%d'.".formatted(lodgingReservationId));
                }

                selectedLodgingReservationId.set(lodgingReservationId);
                ...
                confirmationSimpleBox.addLine("Êtes-vous sûr de vouloir supprimer la réservation d'hébergement " + TextFormatter.bold(selectedLodgingReservation.toString()) + " ?");
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                ...
                if (normalizedInput.equals("N")) {
                    selectedLodgingReservationId.set(-1);
                }
            });

            if (selectedLodgingReservationId.get() == -1) {
                System.out.println(TextFormatter.italic("Suppression annulée."));
                return new GoBackEvent();
            }

            return new FormResultEvent<>(selectedLodgingReservationId.get());
            ...
            if (response instanceof SortColumnCommand sortColumnCommand) {
                return new CallUrlEvent("/camps/manage/%d/lodgings/%d/reservations/delete/sort/%s".formatted(this.campId, this.lodgingId, this.buildSortPathSegment(sortColumnCommand)));
