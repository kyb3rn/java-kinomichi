...
public class DeleteLodgingView extends View {

    // ─── Properties ─── //

    private final int campId;
    private final List<Lodging> sortedLodgings;
    private final LodgingDataManager lodgingDataManager;

    // ─── Constructors ─── //

    public DeleteLodgingView(int campId, List<Lodging> sortedLodgings, LodgingDataManager lodgingDataManager) {
        this.campId = campId;
        this.sortedLodgings = sortedLodgings;
        this.lodgingDataManager = lodgingDataManager;
    }
    ...
    public Event render() {
        ...
        try {
            Table table = ModelTableFormatter.forList(this.sortedLodgings);
            System.out.println();
            table.display();
        ...
        System.out.println(TextFormatter.italic("Entrez l'identifiant (#) d'un hébergement pour le supprimer."));

        try {
            AtomicInteger selectedLodgingId = new AtomicInteger();
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                switch (command) {
                    ...
                    case SortColumnCommand sortColumnCommand -> {
                        int columnCount;
                        try {
                            columnCount = ModelTableFormatter.getColumnCount(Lodging.class);
                        } catch (UnimplementedModelTableException e) {
                ...
                int lodgingId;
                try {
                    lodgingId = Integer.parseInt(input);
                ...
                Lodging selectedLodging = this.lodgingDataManager.getLodgingWithExceptions(lodgingId);

                if (selectedLodging == null) {
                    throw new InvalidMenuInputException("Aucun hébergement ne porte l'identifiant '%d'.".formatted(lodgingId));
                }

                selectedLodgingId.set(lodgingId);
                ...
                confirmationSimpleBox.addLine("Êtes-vous sûr de vouloir supprimer l'hébergement " + TextFormatter.bold(selectedLodging.toString()) + " ?");
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                ...
                if (normalizedInput.equals("N")) {
                    selectedLodgingId.set(-1);
                }
            });

            if (selectedLodgingId.get() == -1) {
                System.out.println(TextFormatter.italic("Suppression annulée."));
                return new GoBackEvent();
            }

            return new FormResultEvent<>(selectedLodgingId.get());
            ...
            if (response instanceof SortColumnCommand sortColumnCommand) {
                return new CallUrlEvent("/camps/manage/%d/lodgings/delete/select/sort/%s".formatted(this.campId, this.buildSortPathSegment(sortColumnCommand)));
