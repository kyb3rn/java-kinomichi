...
public class DeleteAffiliationView extends View {
    ...
    private final List<Affiliation> sortedAffiliations;
    private final AffiliationDataManager affiliationDataManager;

    // ─── Constructors ─── //

    public DeleteAffiliationView(List<Affiliation> sortedAffiliations, AffiliationDataManager affiliationDataManager) {
        this.sortedAffiliations = sortedAffiliations;
        this.affiliationDataManager = affiliationDataManager;
    }
    ...
    public Event render() {
        ...
        try {
            Table table = ModelTableFormatter.forList(this.sortedAffiliations);
            System.out.println();
            table.display();
        ...
        try {
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                switch (command) {
                    ...
                    case SortColumnCommand sortColumnCommand -> {
                        int columnCount;
                        try {
                            columnCount = ModelTableFormatter.getColumnCount(Affiliation.class);
                        } catch (UnimplementedModelTableException e) {
                ...
                int affiliationId;
                try {
                    affiliationId = Integer.parseInt(input);
                ...
                confirmationSimpleBox.addLine("Êtes-vous sûr de vouloir supprimer l'affiliation " + TextFormatter.bold(selectedAffiliation.toString()) + " ?");
            ...
            if (response instanceof SortColumnCommand sortColumnCommand) {
                return new CallUrlEvent("/affiliations/delete/select/sort/" + this.buildSortPathSegment(sortColumnCommand));
