...
public class AddSessionRegistrationView extends FormView {
    ...
    public AddSessionRegistrationView(int campId, int sessionId) {
        this.campId = campId;
    ...
    public Event render() {
        ...
        try {
            DataManagers.get(SessionDataManager.class).getSessionWithExceptions(this.sessionId);
            sessionRegistration.setSessionFromPk(this.sessionId);
        ...
        sectionHeaderSimpleBox.addLine(TextFormatter.bold(TextFormatter.magenta("# Ajout d'une inscription à la session " + TextFormatter.bold("#%d".formatted(this.sessionId)) + " du stage " + TextFormatter.bold("#%d".formatted(this.campId)))));
        ...
        fieldHandlers.put(Field.PERSON_ID, new FieldHandler(TextFormatter.bold(TextFormatter.green("1.")) + " Identifiant de la personne (#)", sessionRegistration::setPersonFromPk));
        ...
        try {
            ...
            confirmationSimpleBox.addLine(TextFormatter.bold(TextFormatter.magenta("# Voulez-vous ajouter cette inscription ?")));
            ...
            do {
                System.out.println();
                Table sessionRegistrationTable = getModelTable(sessionRegistration);
                if (sessionRegistrationTable != null) {
                    sessionRegistrationTable.display();
                }
            ...
            if (input.equals("N")) {
                System.out.println(Functions.styleAsErrorMessage("Ajout d'une inscription annulé."));
                return new GoBackEvent();
            }

            return new FormResultEvent<>(sessionRegistration);
