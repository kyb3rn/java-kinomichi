...
public class DeleteSessionRegistrationView extends View {

    // ─── Properties ─── //

    private final int campId;
    private final int sessionId;
    private final List<SessionRegistration> sortedSessionRegistrations;
    private final SessionRegistrationDataManager sessionRegistrationDataManager;

    // ─── Constructors ─── //

    public DeleteSessionRegistrationView(int campId, int sessionId, List<SessionRegistration> sortedSessionRegistrations, SessionRegistrationDataManager sessionRegistrationDataManager) {
        this.campId = campId;
        this.sessionId = sessionId;
        this.sortedSessionRegistrations = sortedSessionRegistrations;
        this.sessionRegistrationDataManager = sessionRegistrationDataManager;
    }
    ...
    public Event render() {
        ...
        try {
            Table table = ModelTableFormatter.forList(this.sortedSessionRegistrations);
            System.out.println();
            table.display();
        ...
        System.out.println(TextFormatter.italic("Entrez l'identifiant (#) d'une inscription pour la supprimer."));

        try {
            AtomicInteger selectedSessionRegistrationId = new AtomicInteger();
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                switch (command) {
                    ...
                    case SortColumnCommand sortColumnCommand -> {
                        int columnCount;
                        try {
                            columnCount = ModelTableFormatter.getColumnCount(SessionRegistration.class);
                        } catch (UnimplementedModelTableException e) {
                ...
                try {
                    sessionRegistrationId = Integer.parseInt(input);
                ...
                SessionRegistration selectedSessionRegistration = this.sessionRegistrationDataManager.getSessionRegistrationWithExceptions(sessionRegistrationId);

                if (selectedSessionRegistration == null) {
                    throw new InvalidMenuInputException("Aucune inscription ne porte l'identifiant '%d'.".formatted(sessionRegistrationId));
                }

                selectedSessionRegistrationId.set(sessionRegistrationId);
                ...
                confirmationSimpleBox.addLine("Êtes-vous sûr de vouloir supprimer l'inscription " + TextFormatter.bold(selectedSessionRegistration.toString()) + " ?");
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                ...
                if (normalizedInput.equals("N")) {
                    selectedSessionRegistrationId.set(-1);
                }
            });

            if (selectedSessionRegistrationId.get() == -1) {
                System.out.println(TextFormatter.italic("Suppression annulée."));
                return new GoBackEvent();
            }

            return new FormResultEvent<>(selectedSessionRegistrationId.get());
            ...
            if (response instanceof SortColumnCommand sortColumnCommand) {
                return new CallUrlEvent("/camps/manage/%d/sessions/%d/registrations/delete/sort/%s".formatted(this.campId, this.sessionId, this.buildSortPathSegment(sortColumnCommand)));
