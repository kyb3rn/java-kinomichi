...
public class AddSessionTrainerView extends FormView {
    ...
    public AddSessionTrainerView(int campId, int sessionId) {
        this.campId = campId;
    ...
    public Event render() {
        ...
        try {
            ...
            sessionTrainer.setSessionFromPk(this.sessionId);
        ...
        sectionHeaderSimpleBox.addLine(TextFormatter.bold(TextFormatter.magenta("# Ajout d'un formateur à la session " + TextFormatter.bold("#%d".formatted(this.sessionId)) + " du stage " + TextFormatter.bold("#%d".formatted(this.campId)))));
        ...
        fieldHandlers.put(Field.PERSON_ID, new FieldHandler(TextFormatter.bold(TextFormatter.green("1.")) + " Identifiant de la personne (#)", sessionTrainer::setPersonFromPk));
        ...
        try {
            ...
            confirmationSimpleBox.addLine(TextFormatter.bold(TextFormatter.magenta("# Voulez-vous ajouter ce formateur ?")));
            ...
            do {
                System.out.println();
                Table sessionTrainerTable = getModelTable(sessionTrainer);
                if (sessionTrainerTable != null) {
                    sessionTrainerTable.display();
                }
            ...
            if (input.equals("N")) {
                System.out.println(Functions.styleAsErrorMessage("Ajout d'un formateur annulé."));
                return new GoBackEvent();
            }

            return new FormResultEvent<>(sessionTrainer);
