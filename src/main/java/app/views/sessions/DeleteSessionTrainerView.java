...
public class DeleteSessionTrainerView extends View {

    // ─── Properties ─── //

    private final int campId;
    private final int sessionId;
    private final List<SessionTrainer> sortedSessionTrainers;
    private final SessionTrainerDataManager sessionTrainerDataManager;

    // ─── Constructors ─── //

    public DeleteSessionTrainerView(int campId, int sessionId, List<SessionTrainer> sortedSessionTrainers, SessionTrainerDataManager sessionTrainerDataManager) {
        this.campId = campId;
        this.sessionId = sessionId;
        this.sortedSessionTrainers = sortedSessionTrainers;
        this.sessionTrainerDataManager = sessionTrainerDataManager;
    }
    ...
    public Event render() {
        ...
        try {
            Table table = ModelTableFormatter.forList(this.sortedSessionTrainers);
            System.out.println();
            table.display();
        ...
        System.out.println(TextFormatter.italic("Entrez l'identifiant (#) d'un formateur pour le retirer de la session."));

        try {
            AtomicInteger selectedSessionTrainerId = new AtomicInteger();
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                switch (command) {
                    ...
                    case SortColumnCommand sortColumnCommand -> {
                        int columnCount;
                        try {
                            columnCount = ModelTableFormatter.getColumnCount(SessionTrainer.class);
                        } catch (UnimplementedModelTableException e) {
                ...
                try {
                    sessionTrainerId = Integer.parseInt(input);
                ...
                SessionTrainer selectedSessionTrainer = this.sessionTrainerDataManager.getSessionTrainerWithExceptions(sessionTrainerId);

                if (selectedSessionTrainer == null) {
                    throw new InvalidMenuInputException("Aucun formateur ne porte l'identifiant '%d'.".formatted(sessionTrainerId));
                }

                selectedSessionTrainerId.set(sessionTrainerId);

                SimpleBox confirmationSimpleBox = new SimpleBox();
                confirmationSimpleBox.addLine(TextFormatter.bold(TextFormatter.magenta("# Confirmation de retrait")));
                confirmationSimpleBox.addLine("Êtes-vous sûr de vouloir retirer le formateur " + TextFormatter.bold(selectedSessionTrainer.toString()) + " ?");
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                ...
                if (normalizedInput.equals("N")) {
                    selectedSessionTrainerId.set(-1);
                }
            });

            if (selectedSessionTrainerId.get() == -1) {
                System.out.println(TextFormatter.italic("Retrait annulé."));
                return new GoBackEvent();
            }

            return new FormResultEvent<>(selectedSessionTrainerId.get());
            ...
            if (response instanceof SortColumnCommand sortColumnCommand) {
                return new CallUrlEvent("/camps/manage/%d/sessions/%d/trainers/delete/sort/%s".formatted(this.campId, this.sessionId, this.buildSortPathSegment(sortColumnCommand)));
