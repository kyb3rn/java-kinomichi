...
public class AddDinnerView extends FormView {
    ...
    public AddDinnerView(int campId) {
        this.campId = campId;
    }
    ...
    public Event render() {
        ...
        try {
            dinner.setCampFromPk(this.campId);
        } catch (Exception e) {
        ...
        sectionHeaderSimpleBox.addLine(TextFormatter.bold(TextFormatter.magenta("# Ajout d'un repas au stage " + TextFormatter.bold("#%d".formatted(this.campId)))));
        ...
        fieldHandlers.put(Field.LABEL, new FieldHandler(TextFormatter.bold(TextFormatter.green("1.")) + " Nom du repas", dinner::setLabel));
        fieldHandlers.put(Field.TIME_SLOT_START, new FieldHandler(TextFormatter.bold(TextFormatter.green("2.")) + " Date de début " + TextFormatter.italic("(format: yyyy-MM-ddTHH:mm:ssZ)"), input -> {
            ...
            CampScheduledItem.validateInstantWithinCampBounds(dinner.getCamp(), start, "La date de début");
            timeSlotStart.set(start);
            if (timeSlotEnd.get() != null) {
                dinner.setTimeSlot(new TimeSlot(start, timeSlotEnd.get()));
            }
        }));
        fieldHandlers.put(Field.TIME_SLOT_END, new FieldHandler(TextFormatter.bold(TextFormatter.green("3.")) + " Date de fin " + TextFormatter.italic("(format: yyyy-MM-ddTHH:mm:ssZ)"), input -> {
            ...
            CampScheduledItem.validateInstantWithinCampBounds(dinner.getCamp(), end, "La date de fin");
            timeSlotEnd.set(end);
            if (timeSlotStart.get() != null) {
                dinner.setTimeSlot(new TimeSlot(timeSlotStart.get(), end));
            }
        }));
        fieldHandlers.put(Field.PRICE_AMOUNT, new FieldHandler(TextFormatter.bold(TextFormatter.green("4.")) + " Prix " + TextFormatter.italic("(en €)"), input -> {
            double priceAmount = Dinner.verifyPriceAmount(input);
            dinner.setPrice(new Price(Currency.EURO, priceAmount));
        }));
        ...
        try {
            ...
            promptField(scanner, fieldHandlers, Field.PRICE_AMOUNT);

            SimpleBox confirmationSimpleBox = new SimpleBox();
            confirmationSimpleBox.addLine(TextFormatter.bold(TextFormatter.magenta("# Voulez-vous ajouter ce repas ?")));
            ...
            do {
                System.out.println();
                Table dinnerTable = getModelTable(dinner);
                if (dinnerTable != null) {
                    dinnerTable.display();
                }
            ...
            if (input.equals("N")) {
                System.out.println(Functions.styleAsErrorMessage("Ajout d'un repas annulé."));
                return new GoBackEvent();
            }

            return new FormResultEvent<>(dinner);
