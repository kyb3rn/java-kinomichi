...
public class DeleteDinnerReservationView extends View {

    // ─── Properties ─── //

    private final int campId;
    private final int dinnerId;
    private final List<DinnerReservation> sortedDinnerReservations;
    private final DinnerReservationDataManager dinnerReservationDataManager;

    // ─── Constructors ─── //

    public DeleteDinnerReservationView(int campId, int dinnerId, List<DinnerReservation> sortedDinnerReservations, DinnerReservationDataManager dinnerReservationDataManager) {
        this.campId = campId;
        this.dinnerId = dinnerId;
        this.sortedDinnerReservations = sortedDinnerReservations;
        this.dinnerReservationDataManager = dinnerReservationDataManager;
    }
    ...
    public Event render() {
        ...
        try {
            Table table = ModelTableFormatter.forList(this.sortedDinnerReservations);
            System.out.println();
            table.display();
        ...
        System.out.println(TextFormatter.italic("Entrez l'identifiant (#) d'une réservation de repas pour la supprimer."));

        try {
            AtomicInteger selectedDinnerReservationId = new AtomicInteger();
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                switch (command) {
                    ...
                    case SortColumnCommand sortColumnCommand -> {
                        int columnCount;
                        try {
                            columnCount = ModelTableFormatter.getColumnCount(DinnerReservation.class);
                        } catch (UnimplementedModelTableException e) {
                ...
                int dinnerReservationId;
                try {
                    dinnerReservationId = Integer.parseInt(input);
                ...
                DinnerReservation selectedDinnerReservation = this.dinnerReservationDataManager.getDinnerReservationWithExceptions(dinnerReservationId);

                if (selectedDinnerReservation == null) {
                    throw new InvalidMenuInputException("Aucune réservation de repas ne porte l'identifiant '%d'.".formatted(dinnerReservationId));
                }

                selectedDinnerReservationId.set(dinnerReservationId);
                ...
                confirmationSimpleBox.addLine("Êtes-vous sûr de vouloir supprimer l'réservation de repas " + TextFormatter.bold(selectedDinnerReservation.toString()) + " ?");
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                ...
                if (normalizedInput.equals("N")) {
                    selectedDinnerReservationId.set(-1);
                }
            });

            if (selectedDinnerReservationId.get() == -1) {
                System.out.println(TextFormatter.italic("Suppression annulée."));
                return new GoBackEvent();
            }

            return new FormResultEvent<>(selectedDinnerReservationId.get());
            ...
            if (response instanceof SortColumnCommand sortColumnCommand) {
                return new CallUrlEvent("/camps/manage/%d/dinners/%d/reservations/delete/sort/%s".formatted(this.campId, this.dinnerId, this.buildSortPathSegment(sortColumnCommand)));
