...
public class DeleteInvitationView extends View {

    // ─── Properties ─── //

    private final int campId;
    private final List<Invitation> sortedInvitations;
    private final InvitationDataManager invitationDataManager;

    // ─── Constructors ─── //

    public DeleteInvitationView(int campId, List<Invitation> sortedInvitations, InvitationDataManager invitationDataManager) {
        this.campId = campId;
        this.sortedInvitations = sortedInvitations;
        this.invitationDataManager = invitationDataManager;
    }
    ...
    public Event render() {
        ...
        try {
            Table table = ModelTableFormatter.forList(this.sortedInvitations);
            System.out.println();
            table.display();
        ...
        System.out.println(TextFormatter.italic("Entrez l'identifiant (#) d'une invitation pour la supprimer."));

        try {
            AtomicInteger selectedInvitationId = new AtomicInteger();
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                switch (command) {
                    ...
                    case SortColumnCommand sortColumnCommand -> {
                        int columnCount;
                        try {
                            columnCount = ModelTableFormatter.getColumnCount(Invitation.class);
                        } catch (UnimplementedModelTableException e) {
                ...
                int invitationId;
                try {
                    invitationId = Integer.parseInt(input);
                ...
                Invitation selectedInvitation = this.invitationDataManager.getInvitationWithExceptions(invitationId);

                if (selectedInvitation == null) {
                    throw new InvalidMenuInputException("Aucune invitation ne porte l'identifiant '%d'.".formatted(invitationId));
                }

                selectedInvitationId.set(invitationId);
                ...
                confirmationSimpleBox.addLine("Êtes-vous sûr de vouloir supprimer l'invitation " + TextFormatter.bold(selectedInvitation.toString()) + " ?");
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                ...
                if (normalizedInput.equals("N")) {
                    selectedInvitationId.set(-1);
                }
            });

            if (selectedInvitationId.get() == -1) {
                System.out.println(TextFormatter.italic("Suppression annulée."));
                return new GoBackEvent();
            }

            return new FormResultEvent<>(selectedInvitationId.get());
            ...
            if (response instanceof SortColumnCommand sortColumnCommand) {
                return new CallUrlEvent("/camps/manage/%d/invitations/delete/sort/%s".formatted(this.campId, this.buildSortPathSegment(sortColumnCommand)));
