...
public class DeleteCampView extends View {
    ...
    public DeleteCampView(List<Camp> sortedCamps, CampDataManager campDataManager) {
    ...
    public Event render() {
        ...
        System.out.println(TextFormatter.italic("Entrez l'identifiant (#) d'un stage pour le supprimer."));
        ...
        try {
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                ...
                confirmationSimpleBox.addLine("Êtes-vous sûr de vouloir supprimer le stage " + TextFormatter.bold(selectedCamp.toString()) + " ?");
            ...
            KinomichiFunctions.promptInputWithCommandHandling(scanner, (_, command) -> {
                ...
                if (normalizedInput.equals("N")) {
                    selectedCampId.set(-1);
                }
            });

            if (selectedCampId.get() == -1) {
            ...
            if (response instanceof SortColumnCommand sortColumnCommand) {
                return new CallUrlEvent("/camps/delete/sort/" + this.buildSortPathSegment(sortColumnCommand));
