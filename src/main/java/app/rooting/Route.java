...
public class Route {
    ...
    public Route(String name, String pathRegex, ControllerAction controllerAction) {
        ...
        if (pathRegex == null || pathRegex.isBlank()) {
            throw new IllegalArgumentException("Le pattern de chemin de la route '%s' ne peut pas etre vide".formatted(name));
        }

        if (controllerAction == null) {
            throw new IllegalArgumentException("L'action du controller de la route '%s' ne peut pas etre nulle".formatted(name));
        }
