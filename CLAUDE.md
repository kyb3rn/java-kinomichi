...
- Les sections `Getters` / `Setters` sont réservées aux getters et setters **stricts** (accès direct à une propriété). Les méthodes qui délèguent à un sous-objet (ex : `this.paths.size()`) vont dans `Utility methods`.
- Éviter les `if` one-liner sans accolades (ex : `if (x) doSomething();`). Toujours utiliser des accolades. Les ternaires sont OK.
